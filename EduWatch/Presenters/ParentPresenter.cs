...
namespace EduWatch.Presenters
{
    public class ParentPresenter : IParentPresenter
    {
        ...
        internal void OnCancelButtonClick()
        {
            ...
            // Reset data displayed to the user
            view.AutoGenerateDataColumns = true;
            view.ClearData();
            PrepareDataForView();
        }
        ...
        internal void OnSaveButtonClick()
        {
            ...
            if(view.SelectedGradesView)
            {
                var gradesWithChangedSeenStatus = currentGrades.Where(x => x.grade_seen != (dataForGridView as List<Views.GradeViewData>).Where(y => y.ID == x.grade_id).Single().Seen).ToList();
                foreach (var grade in gradesWithChangedSeenStatus)
                    grade.grade_seen = !grade.grade_seen;
            }
            else
            {
                var notesWithChangedSeenStatus = currentNotes.Where(x => x.note_seen != (dataForGridView as List<Views.NoteViewData>).Where(y => y.ID == x.note_Id).Single().Seen).ToList();
